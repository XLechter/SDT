...
from models.model_utils import get_uniform_loss,get_repulsion_loss
...
class FullModel(nn.Module):
    ...
    def forward(self, inputs, gt, eps, iters, EMD=True, CD=True):
        ...
        emd1 = emd2 = cd_p1 = cd_p2 = cd_t1 = cd_t2 = origin_cd_p1 = origin_cd_p2 = torch.tensor([0], dtype=torch.float32).cuda()
        ...
        if CD:
            ...
            # dist31, dist32, _, _ = chamLoss(inputs, output1)
            # dist41, dist42, _, _ = chamLoss(inputs, output2)
            # origin_cd_p1 = (torch.sqrt(dist31).mean(1) + torch.sqrt(dist32).mean(1)) / 2
            # origin_cd_p2 = (torch.sqrt(dist41).mean(1) + torch.sqrt(dist42).mean(1)) / 2


        # u1 = get_uniform_loss(output1)
        # u2 = get_uniform_loss(output2)

        u1 = u2 = torch.tensor([0], dtype=torch.float32).cuda()
        # u2 = get_repulsion_loss(output2)

        return output1, output2, emd1, emd2, cd_p1, cd_p2, cd_t1, cd_t2, u1, u2, origin_cd_p1, origin_cd_p2
